<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<title>Welcome to CodeIgniter 4!</title>
	<meta name="description" content="The small framework with powerful features">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<link rel="shortcut icon" type="image/png" href="/favicon.ico"/>

	<!-- STYLES -->

	<style {csp-style-nonce}>
		* {
			transition: background-color 300ms ease, color 300ms ease;
		}
		*:focus {
			background-color: rgba(221, 72, 20, .2);
			outline: none;
		}
		html, body {
			color: rgba(33, 37, 41, 1);
			font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji";
			font-size: 16px;
			margin: 0;
			padding: 0;
			-webkit-font-smoothing: antialiased;
			-moz-osx-font-smoothing: grayscale;
			text-rendering: optimizeLegibility;
		}
		header {
			background-color: rgba(247, 248, 249, 1);
			padding: .4rem 0 0;
		}
		.menu {
			padding: .4rem 2rem;
		}
		header ul {
			border-bottom: 1px solid rgba(242, 242, 242, 1);
			list-style-type: none;
			margin: 0;
			overflow: hidden;
			padding: 0;
			text-align: right;
		}
		header li {
			display: inline-block;
		}
		header li a {
			border-radius: 5px;
			color: rgba(0, 0, 0, .5);
			display: block;
			height: 44px;
			text-decoration: none;
		}
		header li.menu-item a {
			border-radius: 5px;
			margin: 5px 0;
			height: 38px;
			line-height: 36px;
			padding: .4rem .65rem;
			text-align: center;
		}
		header li.menu-item a:hover,
		header li.menu-item a:focus {
			background-color: rgba(221, 72, 20, .2);
			color: rgba(221, 72, 20, 1);
		}
		header .logo {
			float: left;
			height: 44px;
			padding: .4rem .5rem;
		}
		header .menu-toggle {
			display: none;
			float: right;
			font-size: 2rem;
			font-weight: bold;
		}
		header .menu-toggle button {
			background-color: rgba(221, 72, 20, .6);
			border: none;
			border-radius: 3px;
			color: rgba(255, 255, 255, 1);
			cursor: pointer;
			font: inherit;
			font-size: 1.3rem;
			height: 36px;
			padding: 0;
			margin: 11px 0;
			overflow: visible;
			width: 40px;
		}
		header .menu-toggle button:hover,
		header .menu-toggle button:focus {
			background-color: rgba(221, 72, 20, .8);
			color: rgba(255, 255, 255, .8);
		}
		header .heroe {
			margin: 0 auto;
			max-width: 1100px;
			padding: 1rem 1.75rem 1.75rem 1.75rem;
		}
		header .heroe h1 {
			font-size: 2.5rem;
			font-weight: 500;
		}
		header .heroe h2 {
			font-size: 1.5rem;
			font-weight: 300;
		}
		section {
			margin: 0 auto;
			max-width: 1100px;
			padding: 2.5rem 1.75rem 3.5rem 1.75rem;
		}
		section h1 {
			margin-bottom: 2.5rem;
		}
		section h2 {
			font-size: 120%;
			line-height: 2.5rem;
			padding-top: 1.5rem;
		}
		section pre {
			background-color: rgba(247, 248, 249, 1);
			border: 1px solid rgba(242, 242, 242, 1);
			display: block;
			font-size: .9rem;
			margin: 2rem 0;
			padding: 1rem 1.5rem;
			white-space: pre-wrap;
			word-break: break-all;
		}
		section code {
			display: block;
		}
		section a {
			color: rgba(221, 72, 20, 1);
		}
		section svg {
			margin-bottom: -5px;
			margin-right: 5px;
			width: 25px;
		}
		.further {
			background-color: rgba(247, 248, 249, 1);
			border-bottom: 1px solid rgba(242, 242, 242, 1);
			border-top: 1px solid rgba(242, 242, 242, 1);
		}
		.further h2:first-of-type {
			padding-top: 0;
		}
		footer {
			background-color: rgba(221, 72, 20, .8);
			text-align: center;
		}
		footer .environment {
			color: rgba(255, 255, 255, 1);
			padding: 2rem 1.75rem;
		}
		footer .copyrights {
			background-color: rgba(62, 62, 62, 1);
			color: rgba(200, 200, 200, 1);
			padding: .25rem 1.75rem;
		}
		@media (max-width: 559px) {
			header ul {
				padding: 0;
			}
			header .menu-toggle {
				padding: 0 1rem;
			}
			header .menu-item {
				background-color: rgba(244, 245, 246, 1);
				border-top: 1px solid rgba(242, 242, 242, 1);
				margin: 0 15px;
				width: calc(100% - 30px);
			}
			header .menu-toggle {
				display: block;
			}
			header .hidden {
				display: none;
			}
			header li.menu-item a {
				background-color: rgba(221, 72, 20, .1);
			}
			header li.menu-item a:hover,
			header li.menu-item a:focus {
				background-color: rgba(221, 72, 20, .7);
				color: rgba(255, 255, 255, .8);
			}
		}
	</style>
</head>
<body>

<!-- HEADER: MENU + HEROE SECTION -->
<header>

	<div class="menu">
		<ul>
			<li class="logo"><a href="https://codeigniter.com" target="_blank"><img height="44" title="CodeIgniter Logo"
																					alt="Visit CodeIgniter.com official website!"
																					src="data:image/png;base64,"></a>
			</li>
			<li class="menu-toggle">
				<button onclick="toggleMenu();">&#9776;</button>
			</li>
			<li class="menu-item hidden"><a href="#">Home</a></li>
			<li class="menu-item hidden"><a href="https://codeigniter4.github.io/userguide/" target="_blank">Docs</a>
			</li>
			<li class="menu-item hidden"><a href="https://forum.codeigniter.com/" target="_blank">Community</a></li>
			<li class="menu-item hidden"><a
					href="https://github.com/codeigniter4/CodeIgniter4/blob/develop/CONTRIBUTING.md" target="_blank">Contribute</a>
			</li>
		</ul>
	</div>

	<div class="heroe">
		<h2 style="background-color: maroon;color:aqua; padding:1em;width:50%; font-weight:bold"><?=(session()->getFlashdata('notif'))?session()->getFlashdata('notif'):'';?></h2>
		<h1>Welcome to CodeIgniter <?= CodeIgniter\CodeIgniter::CI_VERSION ?></h1>

		<h2>The small framework with powerful features</h2>
		<?php function check($field, $value){ if (old($field)==$value) {return 'checked';} } ?>
		<?php function Select($field, $value){ if (old($field)==$value) {return 'selected';} } ?>

		<!-- Educational Section -->
		<table border="0" style="margin:3em 0">
			<tr>
				<td>
					<table  border="1" style="border-collapse: collapse;">
						<tr>
							<td>id</td>
							<td>country_collage</td>
							<td>collage_name</td>
							<td>title</td>
							<td>major</td>
							<td>year_graduation</td>
							<td>option</td>
						</tr>
						<?php foreach($educational as $e ): ?>
							<tr>
								<td><?=$e->id?></td>
								<td><?=$e->country_collage?></td>
								<td><?=$e->collage_name?></td>
								<td><?=$e->title?></td>
								<td><?=$e->major?></td>
								<td><?=$e->year_graduation?></td>
								<td><a href="/educational/drop/<?=$e->id?>">Delete</a></td>
							</tr>
						<?php endforeach; ?>
					</table>
				</td>
				<td>
					<h3>Create Educational Form</h3>
					<form action="/educational/save/" method="POST">
						<span>country_collage</span>
						<input type="text" name="country_collage" value="<?=old('country_collage')?>"><br>
						<em><?=$validation->getError('country_collage')?></em>

						<span>collage_name</span>
						<input type="text" name="collage_name" value="<?=old('collage_name')?>"><br>
						<em><?=$validation->getError('collage_name')?></em><br>
						
						<span>title</span>
						<select name="title" id="title">
							<option value="associate" <?=Select('title','associate')?>>
							associate</option>
							<option value="certificate" <?=Select('title','certificate')?>>
							certificate</option>
							<option value="B.A." <?=Select('title','B.A')?>>
							B.A.</option>
							<option value="BArch" <?=Select('title','BArch')?>>
							BArch</option>
							<option value="BM" <?=Select('title','BM')?>>
							BM</option>
							<option value="BFA" <?=Select('title','BFA')?>>
							BFA</option>
							<option value="B.Sc." <?=Select('title','B.Sc.')?>>
							B.Sc.</option>
							<option value="M.A." <?=Select('title','M.A.')?>>
							M.A.</option>
							<option value="M.B.A" <?=Select('title','M.B.A')?>>
							M.B.A</option>
							<option value="M.F.A" <?=Select('title','M.F.A')?>>
							M.F.A</option>
							<option value="M.Sc." <?=Select('title','M.Sc.')?>>
							M.Sc.</option>
							<option value="J.D." <?=Select('title','J.D.')?>>
							J.D.</option>
							<option value="M.D." <?=Select('title','associate')?>>
							M.D.</option>
							<option value="Ph.D" <?=Select('title','M.D.')?>>
							Ph.D</option>
							<option value="LLM" <?=Select('title','LLM')?>>
							LLM</option>
							<option value="other" <?=Select('title','other')?>>
							other</option>
						</select><br>
						<em><?=$validation->getError('title')?></em>
						
						
						<span>major</span>
						<input type="text" name="major" value="<?=old('major')?>"><br>
						<em><?=$validation->getError('major')?></em>

						<span>year_graduation</span>
						<input type="number" name="year_graduation" value="<?=old('year_graduation')?>"><br><br>
						<em><?=$validation->getError('year_graduation')?></em>

						<button type="submit" style="width: 100%; padding:5px">Save</button>
					</form>
				</td>
				<td>
					<h3>Edit Educational Form</h3>
					<form action="/educational/save/" method="POST">
						<span>id</span>
						<input type="number" name="id" maxl="3"><br>
						<span>country_collage</span>
						<input type="text" name="country_collage"><br>
						<span>collage_name</span>
						<input type="text" name="collage_name"><br>
						<span>title</span>
						<select name="title" id="title">
							<option value="associate">associate</option>
							<option value="certificate">certificate</option>
							<option value="B.A.">B.A.</option>
							<option value="BArch">BArch</option>
							<option value="BM">BM</option>
							<option value="BFA">BFA</option>
							<option value="B.Sc.">B.Sc.</option>
							<option value="M.A.">M.A.</option>
							<option value="M.B.A">M.B.A</option>
							<option value="M.F.A">M.F.A</option>
							<option value="M.Sc.">M.Sc.</option>
							<option value="J.D.">J.D.</option>
							<option value="M.D.">M.D.</option>
							<option value="Ph.D">Ph.D</option>
							<option value="LLM">LLM</option>
							<option value="other">other</option>
						</select><br>
						<span>major</span>
						<input type="text" name="major"><br>
						<span>year_graduation</span>
						<input type="number" name="year_graduation"><br><br>

						<button type="submit" style="width: 100%; padding:5px">Update</button>
					</form>
				</td>
				<td>
					<h3>Get One Data only: Exam:id=1</h3>
					<a href="/educational/findAll/1" target="theframe">Get data id 5</a>
					<iframe name="theframe" frameborder="0" width="300" height="300"></iframe>
				</td>
			</tr>
		</table>
		
		<!-- Personal Section -->
		<h2>Personal Section</h2>
		<table border="0" style="margin:3em 0">
			<tr>
				<td>
					<table border="1" style="border-collapse: collapse;">
						<tr>
							<td>id</td>
							<td>fullname</td>
							<td>born</td>
							<td>AGE</td>
							<td>gender</td>
							<td>country</td>
							<td>about_text</td>
							<td>cv</td>
							<td>option</td>
						</tr>
						<?php foreach($personal as $p ): ?>
							<tr>
								<td><?=$p->id?></td>
								<td><?=$p->fullname?></td>
								<td><?=$p->born?></td>
								<td><?=$p->age?></td>
								<td><?=$p->gender?></td>
								<td><?=$p->country?></td>
								<td><?=$p->about_text?></td>
								<td><?=$p->cv?></td>
								<td><a href="/personal/drop/<?=$p->id?>">Delete</a></td>
							</tr>
						<?php endforeach; ?>
					</table>
				</td>
				<td>
					<h2>Create Personal form</h2>
					<form action="/personal/save/" method="POST" >
						<span>fullname</span>
						<input type="text" name="fullname" value="<?=old('fullname')?>"><br>
						<em><?=$validation->getError('fullname')?></em>
						
						<span>born</span>
						<input type="text" name="born" value="<?=old('born')?>"><br>
						<em><?=$validation->getError('born')?></em>
						
						<span>age</span>
						<input type="number" name="age" value="<?=old('age')?>"><br>
						<em><?=$validation->getError('age')?></em>

						<span>gender</span><br>
						female<input type="radio" name="gender" value="female" <?=check('gender','female')?>><br>
						male<input type="radio" name="gender" value="male" <?=check('gender','male')?>><br>
						<em><?=$validation->getError('gender')?></em>
						
						<span>country</span>
						<input type="text" name="country" value="<?=old('country')?>"><br>
						<em><?=$validation->getError('country')?></em>
						
						<textarea name="about_text"cols="20" rows="10"><?=old('about_text')?></textarea><br>
						<em><?=$validation->getError('about_text')?></em>
						
						<span>cv</span>
						<input type="file" name="cv" value="<?=old('cv')?>"><br>
						<em><?=$validation->getError('cv')?></em>

						<button type="submit" style="width: 100%; padding:5px">Save</button>
					</form>		
				</td>
				<td>
					<h2>Edit Personal form</h2>
					<form action="/personal/save/" method="POST">
						
						<span>id</span>
						<input type="number" name="id"><br>

						<span>fullname</span>
						<input type="text" name="fullname"><br>

						<span>born</span>
						<input type="text" name="born"><br>

						<span>age</span>
						<input type="number" name="age"><br>
						
						<span>gender</span><br>
						female<input type="radio" name="gender" value="female"><br>
						male<input type="radio" name="gender" value="male"><br>

						<span>country</span>
						<input type="text" name="country"><br>
						
						<textarea name="about_text"cols="20" rows="10"></textarea><br>
						
						<span>cv</span>
						<input type="file" name="cv"><br>

						<button type="submit" style="width: 100%; padding:5px">Update</button>
					</form>		
				</td>
				<td>
					<h3>Get One Data only: Exam:id=1</h3>
					<a href="/personal/findAll/1" target="theframe5">Get data id</a>
					<iframe name="theframe5" frameborder="0" width="300" height="300"></iframe>
				</td>
			</tr>
		</table>

		<!-- Portfolio Section -->
		<h2>Portfolio Section</h2>
		<table border="0" style="margin:3em 0">
			<tr>
				<td>
					<table border="1" style="border-collapse: collapse;">
						<tr>
							<td style="padding:5px">id</td>
							<td style="padding:5px">file_name</td>
							<td style="padding:5px">type</td>
							<td style="padding:5px">captions</td>
							<td style="padding:5px">option</td>
						</tr>
						<?php foreach($portfolio as $p ): ?>
							<tr>
								<td style="padding:5px"><?=$p->id?></td>
								<?php if ($p->type == 'image') :?>
									<td style="padding:5px">
									<img src="<?= base_url() ?>/assets/img/gallery/pic/<?=$p->file_name?>" alt="<?=$p->captions?> " width="200" height="200">
								</td>
								<?php else : ?>
								<td style="padding:5px">
									<iframe width="250" height="115" src="https://www.youtube.com/embed/<?=$p->file_name?>" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>	
								</td>
								<?php endif;?>
								<td style="padding:5px"><?=$p->type?></td>
								<td style="padding:5px"><?=$p->captions?></td>
								<td style="padding:5px"><a href="/portfolio/drop/<?=$p->id?>">Delete</a></td>
							</tr>
						<?php endforeach; ?>
					</table>
				</td>
				<td>
					<h3>Create Portfolio Form</h3>
					<form action="/portfolio/save/" method="POST">
				
						<span>file_name</span>
						<input type="file" name="file_name"><br>
						<?=$validation->getError('file_name')?>
						
						<span>type</span><br>
						image<input type="radio" name="type" value="image" <?=check('type','image')?>><br>
						video<input type="radio" name="type" value="video" <?=check('type','video')?>><br>
						<?=$validation->getError('type')?>
						
						<span>captions</span>
						<input type="text" name="captions" value="<?=old('captions')?>"><br>
						<?=$validation->getError('captions')?>

						<button type="submit" style="width: 100%; padding:5px">Save</button>
					</form>		
				</td>
				<td>
					<h3>Edit Portfolio Form</h3>
					<form action="/portfolio/save/" method="POST">

						<span>id</span>
						<input type="number" name="id"><br>

						<span>file_name</span>
						<input type="file" name="file_name"><br>
						
						<span>type</span><br>
						image<input type="radio" name="type" value="image"><br>
						video<input type="radio" name="type" value="video"><br>

						<span>captions</span>
						<input type="text" name="captions"><br>

						<button type="submit" style="width: 100%; padding:5px">Update</button>
					</form>		
				</td>
				<td>
					<h3>Get One Data only: Exam:id=2</h3>
					<a href="/portfolio/findAll/2" target="theframe4">Get data id</a>
					<iframe name="theframe4" frameborder="0" width="300" height="300"></iframe>
				</td>
			</tr>
		</table>

		<!-- skills Section -->
		<h2>Skills Section</h2>
		<table border="0" style="margin:3em 0">
			<tr>
				<td>
					<table border="1" style="border-collapse: collapse;">
						<tr>
							<td>id</td>
							<td>skill_name</td>
							<td>skill_value</td>
							<td>option</td>
						</tr>
						<?php foreach($skills as $skill ): ?>
							<tr>
								<td><?=$skill->id?></td>
								<td><?=$skill->skill_name?></td>
								<td><?=$skill->skill_value?></td>
								<td><a href="/skills/drop/<?=$skill->id?>">Delete</a></td>
							</tr>
						<?php endforeach; ?>
					</table>
				</td>
				<td>
					<h3>Create Skill Form</h3>
					<form action="/skills/save/" method="POST">

						<span>skill_name</span>
						<input type="text" name="skill_name" value="<?=old('skill_name')?>"><br>
						<?=$validation->getError('skill_name')?>
						
						<span>skill value</span><br>
						<input type="range" name="skill_value" min="1" max="100" value="<?=old('skill_value')?>">

						<button type="submit" style="width: 100%; padding:5px">Save</button>
					</form>		
				</td>
				<td>
					<h3>Edit Skill Form</h3>
					<form action="/skills/save/" method="POST">
						<span>id</span>
						<input type="number" name="id"><br>

						<span>skill_name</span>
						<input type="text" name="skill_name"><br>
						
						<span>skill value</span><br>
						<input type="range" name="skill_value" min="1" max="100">

						<button type="submit" style="width: 100%; padding:5px">Update</button>
					</form>		
				</td>
				<td>
					<h3>Get One Data only: Exam:id=1</h3>
					<a href="/skills/findAll/1" target="theframe3">Get data id</a>
					<iframe name="theframe3" frameborder="0" width="300" height="300"></iframe>
				</td>
			</tr>
		</table>

		<!-- slider Section -->
		<h2>Slider Section</h2>
		<table border="0" style="margin:3em 0">
			<tr>
				<td>
					<table border="1" style="border-collapse: collapse;">
						<tr>
							<td style="padding:5px">id</td>
							<td style="padding:5px">tagline</td>
							<td style="padding:5px">description</td>
							<td style="padding:5px">align</td>
							<td style="padding:5px">image</td>
							<td style="padding:5px">option</td>
						</tr>
						<?php foreach($sliders as $s ): ?>
							<tr>
								<td><?=$s->id?></td>
								<td><?=$s->tagline?></td>
								<td><?=$s->description?></td>
								<td><?=$s->align?></td>
								<td><img src="/img/carousel/<?=$s->img?>" alt="<?=substr($s->tagline, 15);?>" width="200"></td>
								<td><a href="/sliders/drop/<?=$s->id?>">Delete</a></td>
							</tr>
						<?php endforeach; ?>
					</table>
				</td>
				<td>
					<h3>Create Slider Form</h3>
					<form action="/sliders/save/" method="POST">

						<span>tagline</span>
						<input type="text" name="tagline" value="<?=old('tagline')?>"><br>
						<?=$validation->getError('tagline')?>
						
						<span>description</span>
						<input type="text" name="description" value="<?=old('description')?>"><br>
						<?=$validation->getError('description')?>
						
						<span>align</span><br>
						left<input type="radio" name="align" value="left" <?=check('align','left')?>><br>
						right<input type="radio" name="align" value="right" <?=check('align','right')?>><br>
						center<input type="radio" name="align" value="center" <?=check('align','center')?>><br>
						<?=$validation->getError('align')?>
						
						<span>image</span>
						<input type="file" name="img"><br>
						<?=$validation->getError('img')?>

						<button type="submit" style="width: 100%; padding:5px">Save</button>
					</form>		
				</td>
				<td>
					<h3>Edit Slider Form</h3>
					<form action="/sliders/save/" method="POST">
						<span>id</span>
						<input type="number" name="id"><br>
						
						<span>tagline</span>
						<input type="text" name="tagline"><br>
						
						<span>description</span>
						<input type="text" name="description"><br>
						
						<span>align</span><br>
						left<input type="radio" name="align" value="left"><br>
						right<input type="radio" name="align" value="right"><br>
						center<input type="radio" name="align" value="center"><br>
					
						<span>image</span>
						<input type="file" name="img"><br>

						<button type="submit" style="width: 100%; padding:5px">Update</button>
					</form>		
				</td>
				<td>
					<h3>Get One Data only: Exam:id=1</h3>
					<a href="/sliders/findAll/1" target="theframe2">Get data id</a>
					<iframe name="theframe2" frameborder="0" width="300" height="300"></iframe>
				</td>
			</tr>
		</table>
				
		<!-- social_contact Section -->
		<h2>Social_contact Section</h2>
		<table border="0" style="margin:3em 0">
			<tr>
				<td>
					<table border="1" style="border-collapse: collapse;">
						<tr>
							<td>id</td>
							<td>contact_type</td>
							<td>link</td>
							<td>option</td>
						</tr>
						<?php foreach($social as $sc ): ?>
							<tr>
								<td><?=$sc->id?></td>
								<td><?=$sc->contact_type?></td>
								<td><?=$sc->link?></td>
								<td><a href="/social/drop/<?=$sc->id?>">Delete</a></td>
							</tr>
						<?php endforeach; ?>
					</table>
				</td>
				<td>
					<h3>Create Social Contact</h3>
					<form action="/social/save/" method="POST">

						<span>contact_type</span>
						<input type="text" name="contact_type" value="<?=old('contact_type')?>"><br>
						<?=$validation->getError('contact_type')?>
						
						<span>link</span>
						<input type="text" name="link" value="<?=old('link')?>"><br>
						<?=$validation->getError('link')?>

						<button type="submit" style="width: 100%; padding:5px">Save</button>
					</form>		
				</td>
				<td>
					<h3>Edit Social Contact</h3>
					<form action="/social/save/" method="POST">
						<span>id</span>
						<input type="number" name="id"><br>

						<span>contact_type</span>
						<input type="text" name="contact_type"><br>
						
						<span>link</span>
						<input type="text" name="link"><br>

						<button type="submit" style="width: 100%; padding:5px">Update</button>
					</form>		
				</td>
				<td>
					<h3>Get One Data only: Exam:id=1</h3>
					<a href="/social/findAll/1" target="theframe1">Get data id</a>
					<iframe name="theframe1" frameborder="0" width="300" height="300"></iframe>
				</td>
			</tr>
		</table>
	</div>

</header>

<!-- CONTENT -->

<section>

	<h1>About this page</h1>

	<p>The page you are looking at is being generated dynamically by CodeIgniter.</p>

	<p>If you would like to edit this page you will find it located at:</p>

	<pre><code>app/Views/welcome_message.php</code></pre>

	<p>The corresponding controller for this page can be found at:</p>

	<pre><code>app/Controllers/Home.php</code></pre>

</section>

<div class="further">

	<section>

		<h1>Go further</h1>

		<h2>
			<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512'><rect x='32' y='96' width='64' height='368' rx='16' ry='16' style='fill:none;stroke:#000;stroke-linejoin:round;stroke-width:32px'/><line x1='112' y1='224' x2='240' y2='224' style='fill:none;stroke:#000;stroke-linecap:round;stroke-linejoin:round;stroke-width:32px'/><line x1='112' y1='400' x2='240' y2='400' style='fill:none;stroke:#000;stroke-linecap:round;stroke-linejoin:round;stroke-width:32px'/><rect x='112' y='160' width='128' height='304' rx='16' ry='16' style='fill:none;stroke:#000;stroke-linejoin:round;stroke-width:32px'/><rect x='256' y='48' width='96' height='416' rx='16' ry='16' style='fill:none;stroke:#000;stroke-linejoin:round;stroke-width:32px'/><path d='M422.46,96.11l-40.4,4.25c-11.12,1.17-19.18,11.57-17.93,23.1l34.92,321.59c1.26,11.53,11.37,20,22.49,18.84l40.4-4.25c11.12-1.17,19.18-11.57,17.93-23.1L445,115C443.69,103.42,433.58,94.94,422.46,96.11Z' style='fill:none;stroke:#000;stroke-linejoin:round;stroke-width:32px'/></svg>
			Learn
		</h2>

		<p>The User Guide contains an introduction, tutorial, a number of "how to"
			guides, and then reference documentation for the components that make up
			the framework. Check the <a href="https://codeigniter4.github.io/userguide"
			target="_blank">User Guide</a> !</p>

		<h2>
			<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512'><path d='M431,320.6c-1-3.6,1.2-8.6,3.3-12.2a33.68,33.68,0,0,1,2.1-3.1A162,162,0,0,0,464,215c.3-92.2-77.5-167-173.7-167C206.4,48,136.4,105.1,120,180.9a160.7,160.7,0,0,0-3.7,34.2c0,92.3,74.8,169.1,171,169.1,15.3,0,35.9-4.6,47.2-7.7s22.5-7.2,25.4-8.3a26.44,26.44,0,0,1,9.3-1.7,26,26,0,0,1,10.1,2L436,388.6a13.52,13.52,0,0,0,3.9,1,8,8,0,0,0,8-8,12.85,12.85,0,0,0-.5-2.7Z' style='fill:none;stroke:#000;stroke-linecap:round;stroke-miterlimit:10;stroke-width:32px'/><path d='M66.46,232a146.23,146.23,0,0,0,6.39,152.67c2.31,3.49,3.61,6.19,3.21,8s-11.93,61.87-11.93,61.87a8,8,0,0,0,2.71,7.68A8.17,8.17,0,0,0,72,464a7.26,7.26,0,0,0,2.91-.6l56.21-22a15.7,15.7,0,0,1,12,.2c18.94,7.38,39.88,12,60.83,12A159.21,159.21,0,0,0,284,432.11' style='fill:none;stroke:#000;stroke-linecap:round;stroke-miterlimit:10;stroke-width:32px'/></svg>
			Discuss
		</h2>

		<p>CodeIgniter is a community-developed open source project, with several
			 venues for the community members to gather and exchange ideas. View all
			 the threads on <a href="https://forum.codeigniter.com/"
			 target="_blank">CodeIgniter's forum</a>, or <a href="https://codeigniterchat.slack.com/"
			 target="_blank">chat on Slack</a> !</p>

		<h2>
			 <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512'><line x1='176' y1='48' x2='336' y2='48' style='fill:none;stroke:#000;stroke-linecap:round;stroke-miterlimit:10;stroke-width:32px'/><line x1='118' y1='304' x2='394' y2='304' style='fill:none;stroke:#000;stroke-linecap:round;stroke-miterlimit:10;stroke-width:32px'/><path d='M208,48v93.48a64.09,64.09,0,0,1-9.88,34.18L73.21,373.49C48.4,412.78,76.63,464,123.08,464H388.92c46.45,0,74.68-51.22,49.87-90.51L313.87,175.66A64.09,64.09,0,0,1,304,141.48V48' style='fill:none;stroke:#000;stroke-linecap:round;stroke-miterlimit:10;stroke-width:32px'/></svg>
			 Contribute
		</h2>

		<p>CodeIgniter is a community driven project and accepts contributions
			 of code and documentation from the community. Why not
			 <a href="https://codeigniter.com/en/contribute" target="_blank">
			 join us</a> ?</p>

	</section>

</div>

<!-- FOOTER: DEBUG INFO + COPYRIGHTS -->

<footer>
	<div class="environment">

		<p>Page rendered in {elapsed_time} seconds</p>

		<p>Environment: <?= ENVIRONMENT ?></p>

	</div>

	<div class="copyrights">

		<p>&copy; <?= date('Y') ?> CodeIgniter Foundation. CodeIgniter is open source project released under the MIT
			open source licence.</p>

	</div>

</footer>

<!-- SCRIPTS -->

<script>
	function toggleMenu() {
		var menuItems = document.getElementsByClassName('menu-item');
		for (var i = 0; i < menuItems.length; i++) {
			var menuItem = menuItems[i];
			menuItem.classList.toggle("hidden");
		}
	}

</script>

<!-- -->

</body>
</html>
